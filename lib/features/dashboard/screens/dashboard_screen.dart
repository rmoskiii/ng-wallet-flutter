...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    super.initState();
    Future.delayed(const Duration(milliseconds: 500), () => setState(() => showProgress = true));
    Future.delayed(const Duration(seconds: 1), () => setState(() => reminderOpen = true));
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('NG Wallet Dashboard')),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: const [
                        Text('653', style: TextStyle(fontSize: 36, fontWeight: FontWeight.bold)),
                        SizedBox(width: 8),
                        Text('▲ 60', style: TextStyle(color: Colors.green)),
                      ],
                    ),
                    const SizedBox(height: 8),
                    const Text('Next update in 2 days', style: TextStyle(color: Colors.grey)),
                            ...
                            bottomTitles: AxisTitles(
                              sideTitles: SideTitles(
                                ...
                                getTitlesWidget: (value, meta) {
                                  ...
                                  if (index < 0 || index >= scoreData.length) return const Text('');
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('My Savings', style: TextStyle(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    const Text('₦450,000', style: TextStyle(fontSize: 20, color: Colors.green)),
                    const SizedBox(height: 8),
              ...
              child: ExpansionTile(
                ...
                children: breakdownData.map((item) {
                  return ListTile(
                    ...
                    subtitle: Text('Score: ${item['score']}, Grade: ${item['grade']}'),
                  );
                }).toList(),
              ),
            ),

            // How Scores Are Calculated Accordion
            Card(
              margin: const EdgeInsets.only(bottom: 16),
              child: ExpansionTile(
